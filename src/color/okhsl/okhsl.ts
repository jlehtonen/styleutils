...
const getCs = (L: number, a_: number, b_: number) => {
  ...
  let C_max = findGamutIntersection(a_, b_, L, 1, L, cusp);
  let ST_max = getStMax(a_, b_, cusp);
...
const findCusp = (a: number, b: number) => {
  ...
  let rgb_at_max = oklabToLinearSrgb(1, S_cusp * a, S_cusp * b);
